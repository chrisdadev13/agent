...
async function slackMessage(
  ...
  if (response.status >= 500)
    throw new Error("Slack is temporarily unavailable.");
  if (!response.ok)
    throw new FatalError(`Slack returned HTTP ${response.status}.`);

  const result = await response.json();
  if (!result.ok) {
    if (
      ["internal_error", "fatal_error", "ratelimited"].includes(result.error)
    ) {
...
async function requestRefund(
  ...
  using approval = createHook<RefundDecision>({
    token: `refund:${workflowRunId}:${toolCallId}`,
  });
  ...
  const details = [
    ...
    {
      type: "section",
      text: { type: "plain_text", text: `Reason\n${input.reason}` },
    },
    { type: "divider" },
    {
      type: "section",
      text: {
        type: "plain_text",
        text: `Recent customer messages (up to 4, shortened if needed)\n${formatRefundContext(messages)}`,
      },
    },
    {
      type: "context",
      elements: [
        {
          type: "plain_text",
          text: `Requested: ${new Date().toISOString()} · Workflow: ${workflowRunId}`,
        },
      ],
    },
    {
      ...
      elements: [
        {
          type: "plain_text",
          text: "Demo only — approving records a decision; no money is moved.",
        },
      ],
    },
  ];
  ...
  const message = await slackMessage("chat.postMessage", {
    ...
    blocks: [
      ...details,
      {
        ...
        text: {
          type: "plain_text",
          text: "Pending review · Approve or reject this refund request below.",
        },
      },
      {
        type: "actions",
  ...
  await slackMessage("chat.update", {
    ...
    blocks: [
      ...details,
      {
        ...
        text: {
          type: "plain_text",
          text: `${decision.approved ? "Approved" : "Rejected"} by ${decision.reviewer}\nDecision recorded: ${new Date().toISOString()}`,
        },
      },
    ],
  }).catch((error) => {
...
export const refundTools = {
  requestRefund: {
    description:
      "Request human approval in Slack for a refund explicitly requested by the customer. Waits for approval or rejection. This demo does not move money.",

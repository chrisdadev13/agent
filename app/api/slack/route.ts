...
export async function POST(request: Request) {
  ...
  const timestampDifferenceSeconds = Math.abs(Date.now() / 1000 - Number(timestamp))
  ...
  if (!result.success) return new Response('Invalid Slack interaction.', { status: 400 })
  ...
  try {
    ...
    if (!HookNotFoundError.is(error)) {
      ...
      return new Response('Unable to record approval. Please retry.', { status: 500 })
    }

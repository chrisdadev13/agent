...
import { z } from "zod";
import { refundDecisionSchema, refundInputSchema } from "#/lib/refund";
import { chatWorkflow } from "#/workflows/chat";

const refundPartSchema = z
  .object({
...
const messageSchema = z
  ...
  .refine(
    (message) =>
      ...
      message.parts.every((part) => part.type !== "tool-requestRefund"),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return new Response("Unable to start the agent. Please try again.", {
      status: 500,
    });
  }
}
